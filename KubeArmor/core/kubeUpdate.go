	...
	if action == "ADDED" {
		...
		for idx, endPoint := range dm.EndPoints {
			if pod.Metadata["namespaceName"] == endPoint.NamespaceName && pod.Metadata["podName"] == endPoint.EndPointName {
				dm.EndPoints[idx].Labels = map[string]string{}
				dm.EndPoints[idx].Identities = []string{"namespaceName=" + pod.Metadata["namespaceName"]}

				// update labels and identities
				for k, v := range pod.Labels {
					dm.EndPoints[idx].Labels[k] = v
				...
				dm.EndPoints[idx].AppArmorProfiles = []string{}
				...
				for _, containerID := range dm.EndPoints[idx].Containers {
					...
					if !kl.ContainsElement(dm.EndPoints[idx].AppArmorProfiles, container.AppArmorProfile) {
				...
				dm.EndPoints[idx].SELinuxProfiles = map[string]string{}
				dm.EndPoints[idx].HostVolumes = []tp.HostVolumeMount{}

				// update selinux profile names to the endpoint
				for k, v := range pod.Annotations {
					if strings.HasPrefix(k, "selinux-") {
						containerName := strings.Split(k, "selinux-")[1]
						dm.EndPoints[idx].SELinuxProfiles[containerName] = v
					}
				}

				// update host-side volume mounted
				dm.EndPoints[idx].HostVolumes = append(dm.EndPoints[idx].HostVolumes, pod.HostVolumes...)
...
func k8sAuditPolicyMergeMacro(k8sAuditPolicySpec *tp.K8sAuditPolicySpec, macroName string, macroValue string) {
	...
	for i := 0; i < len(k8sAuditPolicySpec.AuditRules); i++ {
		...
		for j := 0; j < len(k8sAuditPolicySpec.AuditRules[i].Tags); j++ {
			expandMacroField(&k8sAuditPolicySpec.AuditRules[i].Tags[j], macroName, macroValue)
		}
...
func k8sAuditPolicyConvert(k8sAuditPolicySpec tp.K8sAuditPolicySpec) ([]tp.AuditPolicy, error) {
	...
	k8sAuditPolicySpec.Selector.Identities = []string{}
	for k, v := range k8sAuditPolicySpec.Selector.MatchLabels {
		k8sAuditPolicySpec.Selector.Identities = append(k8sAuditPolicySpec.Selector.Identities, k+"="+v)
	}

	for i, rule := range k8sAuditPolicySpec.AuditRules {
		// set identities
		auditPolicies[i].Selector.Identities = make([]string, len(k8sAuditPolicySpec.Selector.Identities))
		copy(auditPolicies[i].Selector.Identities, k8sAuditPolicySpec.Selector.Identities)
...
func (dm *KubeArmorDaemon) decodeAuditPolicy(decoder *json.Decoder) {
	for {
		...
		if err := kl.Clone(event.Object, &k8sPolicy); err != nil {
			dm.Logger.Errf("Failed to clone spec for Audit Policy (%s/%s). This event will be lost", strings.ToLower(event.Type), k8sPolicy.Metadata.Name)
			continue
		}
		...
		if event.Type == "ADDED" {
			...
			for _, policy := range dm.K8sAuditPolicies {
				if policy.Metadata.Namespace == k8sPolicy.Metadata.Namespace && policy.Metadata.Name == k8sPolicy.Metadata.Name {
					new = false
					break
				}
			}
		...
		dm.Logger.Printf("Detected an Audit Policy (%s/%s)", strings.ToLower(event.Type), k8sPolicy.Metadata.Name)
...
func (dm *KubeArmorDaemon) decodeKubeArmorMacro(decoder *json.Decoder) {
	for {
		...
		macro := tp.K8sKubeArmorMacro{}

		if err := kl.Clone(event.Object, &macro); err != nil {
			dm.Logger.Errf("Failed to clone spec for Audit Policy (%s/%s). This event will be lost", strings.ToLower(event.Type), macro.Metadata.Name)
			continue
		}
		...
		if event.Type == "ADDED" {
			new := true
			for _, k8sMacro := range dm.K8sMacros {
				if macro.Metadata.Namespace == k8sMacro.Metadata.Namespace && macro.Metadata.Name == k8sMacro.Metadata.Name {
					new = false
					break
				}
			}
			if new {
				dm.K8sMacros = append(dm.K8sMacros, macro)
			}
		} else if event.Type == "MODIFIED" {
			for i, k8sMacro := range dm.K8sMacros {
				if macro.Metadata.Namespace == k8sMacro.Metadata.Namespace && macro.Metadata.Name == k8sMacro.Metadata.Name {
		...
		dm.Logger.Printf("Detected a Macro (%s/%s)", strings.ToLower(event.Type), event.Object.Metadata.Name)
